...
class QMC5883L(object):
    ...
    def __init__(self,
        ...
        """
        TODO: explain allowed values in ValueError
        """
    ...
    def get_magnet(self):
        x_data = self.bus.read_i2c_block_data(self.adress, REG_OUT_X_LSB, 6)
        y_data = self.bus.read_i2c_block_data(self.adress, REG_OUT_Y_LSB, 6)
        z_data = self.bus.read_i2c_block_data(self.adress, REG_OUT_Z_LSB, 6)
        x = self._convert_data(x_data, REG_OUT_X_LSB)
        y = self._convert_data(y_data, REG_OUT_Y_LSB)
        z = self._convert_data(z_data, REG_OUT_Z_LSB)
        return [x, y, z]
